...
app.post('/api/daily-reward/claim', authenticateToken, async (req, res) => {
  ...
  const user = await User.findById(req.user.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (user.robloxUsername?.startsWith('Guest_') || user.isGuest === true) return res.status(403).json({ error: 'Guest users can’t earn Passly Coins. Please log in with Roblox to start earning.' });
  const today = getTodayKey();
  const reward = user.dailyReward || {};
