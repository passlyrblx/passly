...
const staticFileOptions = {
  ...
  setHeaders(res, filePath) {
    ...
    if (fileName === 'sitemap.xml') res.setHeader('Content-Type', 'application/xml');
...
app.post('/api/profile/update', authenticateToken, async (req, res) => {
  const User = getUserModel();
  if (!User) return res.status(503).json({ error: 'Database not ready' });
  const { showBooth, showFindPlayerDetails, statusDot, showRoomId, showDiscordIdentity, customDisplayName, displayTag, appTheme } = req.body;
  const user = await User.findById(req.user.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  const update = {};
